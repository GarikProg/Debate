...
router.route('/loading').post(async (req, res) => {
  if (req.session.user) {
    const user = await User.findById(req.session.user.id).populate('threads').populate('comments').populate('debates').populate('wonThreads').populate('wonDebates').populate('votedFor');
